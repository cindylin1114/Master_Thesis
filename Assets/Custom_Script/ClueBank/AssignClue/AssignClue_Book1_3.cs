...
public class AssignClue_Book1_3 : MonoBehaviour // 透過點擊蒐集關鍵字至線索庫中(附加在 keyword_area1_1 - keyword_area1_1_Content - keyword_area1_1_Button)
{
    ...
    [HeaderAttribute("Clue Transform")] // Book_Transform - Book3_Transform 線索庫中關鍵字放置的實際初始位置
    ...
    {
        if (gameManager.ClueNum_Book3 < TargetNum) // 如果當前蒐集數量小於蒐集數量上限
        {
            gameManager.Book3_Keyword1 = true; // 該關鍵字的蒐集狀態 : true

            Orginal_Clue[0].SetActive(false); // 隱藏書籍區中的關鍵字

            Collected_Clue[0].SetActive(true); // 顯示線索庫中的關鍵字

            Collected_Clue[0].transform.position = Clue_Trans[gameManager.ClueNum_Book3].position; // 該關鍵字分配至指定的線索庫位置

            gameManager.ClueNum_Book3++; // 關鍵字蒐集數量 + 1
        } 
        else
        {
            Debug.Log("Book3_Clue is enough!"); // 關鍵字數量已達上限
