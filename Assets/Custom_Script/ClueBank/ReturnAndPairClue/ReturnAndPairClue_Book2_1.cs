...
public class ReturnAndPairClue_Book2_1 : MonoBehaviour // 用來將置於線索庫的「關鍵字」配對到對應的測驗題目；將關鍵字放回初始位置(歸位至書籍區) (附加在 Clue_bank - Clue_Bank_Content - keyword_area1_1_Collection)
{
    ...
    public GameObject CheckBoard_Total; // 區域測驗完成的提示視窗
    ...
    public Image BookImage1; // 提示庫中的「書籍」圖樣位置

    public Image BookImage2; // 提示庫中的「書籍」圖樣位置

    private string BookName = "《中國天主教史人物傳》\n〈追憶徐誠斌主教〉";

    [SerializeField] private Sprite Book_6_Sprite_1; // 配對書籍封面

    [SerializeField] private Sprite Book_6_Sprite_2; // 配對書籍封面

    [SerializeField] private Sprite UnknownSprite; // 未解鎖圖片

    [SerializeField] private Sprite CompleteSprite1; // 完成配對書籍封面
    ...
    public static int limitNum = 3; // 指定數量為6個(6個填答區域)
    ...
    void Update()
    {
        ...
        // 開啟「重選線索」功能
        if (gameManager.ClueNum_Book6 > 0 && !gameManager.FinishExam_Book6) // 有關鍵字在線索庫中才會有「重選線索」功能
        {
        ...
        if (gameManager.ClueNum_Book6 >= limitNum) // 如果關鍵字蒐集數量達到指定要求(目前設定需要蒐集"限制數量"關鍵字)
        {
            BookTitle.text = BookName + "\n(完成)"; // 更動書籍標題

            BookTitle.color = Color.green; // 字體顏色變為綠色

            BookImage1.sprite = Book_6_Sprite_1; // 於提示庫中顯示配對書籍封面

            BookImage2.sprite = Book_6_Sprite_2;

            gameManager.ClueComplete_Book6 = true; // 記錄該書的線索已經蒐集完畢
        } 
        else
        {
            BookTitle.text = BookName + "\n(" + gameManager.ClueNum_Book6 + "/" + limitNum + ")"; // 顯示關鍵字蒐集進度

            BookTitle.color = Color.black;

            BookImage1.sprite = UnknownSprite;

            gameManager.ClueComplete_Book6 = false;

            CheckPoint.SetActive(false); // 未達成蒐集數量，書籍測驗畫面不會開啟
        }


        if (gameManager.ClueComplete_Book6 && gameManager.ClueComplete_Book7 && gameManager.ClueComplete_Book8)
        {
        ...
        if (gameManager.Book6_Question1 && gameManager.Book6_Question2 && gameManager.Book6_Question3) // 如果在CheckPoint視窗中，書籍測驗中的問題全部回答正確後
        {
            BookImage1.sprite = CompleteSprite1; // 提示庫中的書籍封面變更為「完成配對書籍封面」

            BookImage2.sprite = CompleteSprite2;

            if (!gameManager.check_flag_6)
            {
        ...
        if (gameManager.FinishExam_Book6 && gameManager.FinishExam_Book7 && gameManager.FinishExam_Book8)
        {
            ...
            if (!gameManager.check_flag_12)
            {
                CheckBoard_Total.SetActive(true);
            }
        }
    }
    ...
    {
        Debug.Log("Reselect the Clue from Book6");
    ...
    {
        Debug.Log("Lock the Clue from Book6");
    ...
    {
        Debug.Log("Return Book6_Keyword1");

        if (gameManager.ClueNum_Book6 > 0) // 如果未達指定關鍵字蒐集數量
        {
            gameManager.Book6_Keyword1 = false; // 該關鍵字的蒐集狀態 : false

            Collected_Clue[0].SetActive(false); // 隱藏線索庫中的關鍵字

            Orginal_Clue[0].SetActive(true); // 顯示書籍區中的關鍵字(原本蒐集後已經變為隱藏狀態)

            gameManager.ClueNum_Book6--; // 關鍵字蒐集數量 - 1

            Clue_Trans[gameManager.ClueNum_Book6].position = Collected_Clue[0].transform.position;
    ...
    {
        Debug.Log("Return Book6_Keyword2");

        if (gameManager.ClueNum_Book6 > 0)
        {
            gameManager.Book6_Keyword2 = false;

            Collected_Clue[1].SetActive(false);

            Orginal_Clue[1].SetActive(true);

            gameManager.ClueNum_Book6--;
            
            Clue_Trans[gameManager.ClueNum_Book6].position = Collected_Clue[1].transform.position;
    ...
    {
        Debug.Log("Return Book6_Keyword3");

        if (gameManager.ClueNum_Book6 > 0)
        {
            gameManager.Book6_Keyword3 = false;
            ...
            Clue_Trans[gameManager.ClueNum_Book6].position = Collected_Clue[2].transform.position;
    ...
    {
        Debug.Log("Return Book6_Keyword4");

        if (gameManager.ClueNum_Book6 > 0)
        {
            gameManager.Book6_Keyword4 = false;
            ...
            Clue_Trans[gameManager.ClueNum_Book6].position = Collected_Clue[3].transform.position;
    ...
    {
        Debug.Log("Return Book6_Keyword5");

        if (gameManager.ClueNum_Book6 > 0)
        {
            gameManager.Book6_Keyword5 = false;
            ...
            Clue_Trans[gameManager.ClueNum_Book6].position = Collected_Clue[4].transform.position;
    ...
    {
        Debug.Log("Book6_Question1 is correct");
        ...
        gameManager.Book6_Question1 = true; // 書籍1的第一個問題已回答正確
        ...
        {
            Debug.Log("Finish Book_6_Exam!");
        }
    }

    void ResetThePos_1() // 回答錯誤
    {
        Debug.Log("Book6_Question1 is wrong");
    ...
    public void ClueDetermine_2() // follow with Clue_list
    {
        ...
        { // pair the answer
            ...
            {
                Debug.Log("Not the request keyword(Book6_Keyword2)");
    ...
    public void ClueDetermine_3() // follow with Clue_list
    {
        if (CheckPoint.activeSelf == true)
        { // pair the answer
            if (Vector3.Distance(Collected_Clue[2].transform.position, Detect_Area[1].transform.position) < snapDistance || Vector3.Distance(Collected_Clue[2].transform.position, Detect_Area[2].transform.position) < snapDistance)
            {
                SnapToCorrectPos_3(); // Clue_1 is paired correctly
                audioSource.volume = successVolume;
                audioSource.PlayOneShot(sound_Success);
            }
            else if (Vector3.Distance(Collected_Clue[2].transform.position, Detect_Area[1].transform.position) >= snapDistance || Vector3.Distance(Collected_Clue[2].transform.position, Detect_Area[2].transform.position) >= snapDistance)
            {
                ResetThePos_3();
                audioSource.volume = failVolume;
                audioSource.PlayOneShot(sound_Fail);
    ...
    void SnapToCorrectPos_3() // Right Answer 
    {
        Debug.Log("Book6_Question2 is correct");
        ...
        if (step < Detect_Area.Count) // if you finish all the part
        {
            step++;
        }
        if (step == Detect_Area.Count)
    ...
    void ResetThePos_3() // Wrong Answer 
    {
        Debug.Log("Book6_Question2 is wrong");
    ...
    public void ClueDetermine_4() // follow with Clue_list
    {
        if (CheckPoint.activeSelf == true)
        { // pair the answer
            if (Vector3.Distance(Collected_Clue[3].transform.position, Detect_Area[1].transform.position) < snapDistance || Vector3.Distance(Collected_Clue[3].transform.position, Detect_Area[2].transform.position) < snapDistance)
            {
                SnapToCorrectPos_4(); // Clue_1 is paired correctly
                audioSource.volume = successVolume;
                audioSource.PlayOneShot(sound_Success);
            }
            else if (Vector3.Distance(Collected_Clue[3].transform.position, Detect_Area[1].transform.position) >= snapDistance || Vector3.Distance(Collected_Clue[3].transform.position, Detect_Area[2].transform.position) >= snapDistance)
            {
                ResetThePos_4();
                audioSource.volume = failVolume;
                audioSource.PlayOneShot(sound_Fail);
    ...
    void SnapToCorrectPos_4() // Right Answer 
    {
        Debug.Log("Book6_Question3 is correct");
    ...
    void ResetThePos_4() // Wrong Answer 
    {
        Debug.Log("Book6_Question3 is wrong");
    ...
    public void ClueDetermine_5() // follow with Clue_list
    {
        ...
        { // pair the answer
            ...
            {
                Debug.Log("Not the request keyword(Book6_Keyword5)");
