...
public class ReturnAndPairClue_Book2_3 : MonoBehaviour // 用來將置於線索庫的「關鍵字」配對到對應的測驗題目；將關鍵字放回初始位置(歸位至書籍區) (附加在 Clue_bank - Clue_Bank_Content - keyword_area1_1_Collection)
{
    ...
    private string BookName = "〈于樞機功在教會〉\n〈于樞機功在國家〉";

    [SerializeField] private Sprite Book_8_Sprite_1; // 配對書籍封面

    [SerializeField] private Sprite Book_8_Sprite_2; // 配對書籍封面
    ...
    void Update()
    {
        ...
        // 開啟「重選線索」功能
        if (gameManager.ClueNum_Book8 > 0 && !gameManager.FinishExam_Book8) // 有關鍵字在線索庫中才會有「重選線索」功能
        {
        ...
        if (gameManager.ClueNum_Book8 >= limitNum) // 如果關鍵字蒐集數量達到指定要求(目前設定需要蒐集"限制數量"關鍵字)
        {
            BookTitle.text = BookName + "\n(完成)"; // 更動書籍標題

            BookTitle.color = Color.green; // 字體顏色變為綠色

            BookImage1.sprite = Book_8_Sprite_1; // 於提示庫中顯示配對書籍封面

            BookImage2.sprite = Book_8_Sprite_2;

            gameManager.ClueComplete_Book8 = true; // 記錄該書的線索已經蒐集完畢
        } 
        else
        {
            BookTitle.text = BookName + "\n(" + gameManager.ClueNum_Book8 + "/" + limitNum + ")"; // 顯示關鍵字蒐集進度

            BookTitle.color = Color.black;

            BookImage1.sprite = UnknownSprite;

            gameManager.ClueComplete_Book8 = false;

            CheckPoint.SetActive(false); // 未達成蒐集數量，書籍測驗畫面不會開啟
        }

        if (gameManager.Book8_Question1 && gameManager.Book8_Question2 && gameManager.Book8_Question3) // 如果在CheckPoint視窗中，書籍測驗中的問題全部回答正確後
        {
            BookImage1.sprite = CompleteSprite1; // 提示庫中的書籍封面變更為「完成配對書籍封面」

            BookImage2.sprite = CompleteSprite2;

            if (!gameManager.check_flag_8)
            {
    ...
    {
        Debug.Log("Reselect the Clue from Book8");
    ...
    {
        Debug.Log("Lock the Clue from Book8");
    ...
    {
        Debug.Log("Return Book8_Keyword1");

        if (gameManager.ClueNum_Book8 > 0) // 如果未達指定關鍵字蒐集數量
        {
            gameManager.Book8_Keyword1 = false; // 該關鍵字的蒐集狀態 : false

            Collected_Clue[0].SetActive(false); // 隱藏線索庫中的關鍵字

            Orginal_Clue[0].SetActive(true); // 顯示書籍區中的關鍵字(原本蒐集後已經變為隱藏狀態)

            gameManager.ClueNum_Book8--; // 關鍵字蒐集數量 - 1

            Clue_Trans[gameManager.ClueNum_Book8].position = Collected_Clue[0].transform.position;
    ...
    {
        Debug.Log("Return Book8_Keyword2");

        if (gameManager.ClueNum_Book8 > 0)
        {
            gameManager.Book8_Keyword2 = false;

            Collected_Clue[1].SetActive(false);

            Orginal_Clue[1].SetActive(true);

            gameManager.ClueNum_Book8--;
            
            Clue_Trans[gameManager.ClueNum_Book8].position = Collected_Clue[1].transform.position;
    ...
    {
        Debug.Log("Return Book8_Keyword3");

        if (gameManager.ClueNum_Book8 > 0)
        {
            gameManager.Book8_Keyword3 = false;
            ...
            Clue_Trans[gameManager.ClueNum_Book8].position = Collected_Clue[2].transform.position;
    ...
    {
        Debug.Log("Return Book8_Keyword4");

        if (gameManager.ClueNum_Book8 > 0)
        {
            gameManager.Book8_Keyword4 = false;
            ...
            Clue_Trans[gameManager.ClueNum_Book8].position = Collected_Clue[3].transform.position;
    ...
    {
        Debug.Log("Return Book8_Keyword5");

        if (gameManager.ClueNum_Book8 > 0)
        {
            gameManager.Book8_Keyword5 = false;
            ...
            Clue_Trans[gameManager.ClueNum_Book8].position = Collected_Clue[4].transform.position;
    ...
    {
        Debug.Log("Book8_Question1 is correct");
        ...
        gameManager.Book8_Question1 = true; // 書籍1的第一個問題已回答正確
        ...
        {
            Debug.Log("Finish Book_8_Exam!");
        }
    }

    void ResetThePos_1() // 回答錯誤
    {
        Debug.Log("Book8_Question1 is wrong");
    ...
    // 〈于樞機功在教會〉〈于樞機功在國家〉 第二個關鍵字
    public void ClueDetermine_2() // 用以判斷關鍵字與書籍測驗回答區域之間的互動 (for每一個問題)
    {
        if (CheckPoint.activeSelf == true)
        { // pair the answer
            if (Vector3.Distance(Collected_Clue[1].transform.position, Detect_Area[1].transform.position) < snapDistance || Vector3.Distance(Collected_Clue[1].transform.position, Detect_Area[2].transform.position) < snapDistance) // 如果(線索庫)關鍵字與目標偵測回答區域"小於"吸取距離
            {
                SnapToCorrectPos_2(); // 回答正確的後續處理
                audioSource.volume = successVolume;
                audioSource.PlayOneShot(sound_Success); // 播放"成功"的音效
            }
            else if (Vector3.Distance(Collected_Clue[1].transform.position, Detect_Area[1].transform.position) >= snapDistance || Vector3.Distance(Collected_Clue[1].transform.position, Detect_Area[2].transform.position) >= snapDistance) // 如果(線索庫)關鍵字與目標偵測回答區域"不小於"吸取距離
            {
    ...
    {
        Debug.Log("Book8_Question2 is correct");
        ...
        gameManager.Book8_Question2 = true; // 書籍1的第四個問題已回答正確
    ...
    {
        Debug.Log("Book8_Question2 is wrong");
    ...
    {
        Debug.Log("Book8_Question3 is correct");
    ...
    {
        Debug.Log("Book8_Question3 is wrong");
    ...
    public void ClueDetermine_4() // follow with Clue_list
    {
        ...
        { // pair the answer
            ...
            {
                Debug.Log("Not the request keyword(Book8_Keyword4)");
    ...
    public void ClueDetermine_5() // follow with Clue_list
    {
        ...
        { // pair the answer
            ...
            {
                Debug.Log("Not the request keyword(Book8_Keyword5)");
