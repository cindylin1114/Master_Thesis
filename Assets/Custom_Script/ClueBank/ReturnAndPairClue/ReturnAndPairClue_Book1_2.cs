...
public class ReturnAndPairClue_Book1_2 : MonoBehaviour // 用來將置於線索庫的「關鍵字」配對到對應的測驗題目；將關鍵字放回初始位置(歸位至書籍區) (附加在 Clue_bank - Clue_Bank_Content - keyword_area1_1_Collection)
{
    ...
    void Update()
    {
        ...
        if (gameManager.ClueNum_Book2 > 0 && !gameManager.FinishExam_Book2) // 有關鍵字在線索庫中才會有「重選線索」功能
        {
        ...
        if (gameManager.ClueNum_Book2 >= limitNum) // 如果關鍵字蒐集數量達到指定要求(目前設定需要蒐集"限制數量"關鍵字)
        {
            ...
            gameManager.ClueComplete_Book2 = true; // 記錄該書的線索已經蒐集完畢
        } 
        else
        {
            BookTitle.text = BookName + "(" + gameManager.ClueNum_Book2 + "/" + limitNum + ")"; // 顯示關鍵字蒐集進度
        ...
        if (gameManager.Book2_Question1 && gameManager.Book2_Question2 && gameManager.Book2_Question3 && gameManager.Book2_Question4 && gameManager.Book2_Question5 && gameManager.Book2_Question6) // 如果在CheckPoint視窗中，書籍測驗中的問題全部回答正確後
        {
    ...
    public void Keyword1_return() // 關鍵字歸位 功能 (for每一個關鍵字)
    {
        ...
        if (gameManager.ClueNum_Book2 > 0) // 如果未達指定關鍵字蒐集數量
        {
            gameManager.Book2_Keyword1 = false; // 該關鍵字的蒐集狀態 : false
    ...
    void SnapToCorrectPos_1() // 回答正確
    {
        ...
        gameManager.Book2_Question1 = true; // 書籍1的第一個問題已回答正確
    ...
    void SnapToCorrectPos_2() // 回答正確
    {
        ...
        gameManager.Book2_Question2 = true; // 書籍1的第四個問題已回答正確
