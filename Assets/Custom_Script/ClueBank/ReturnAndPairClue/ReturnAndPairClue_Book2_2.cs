...
public class ReturnAndPairClue_Book2_2 : MonoBehaviour // 用來將置於線索庫的「關鍵字」配對到對應的測驗題目；將關鍵字放回初始位置(歸位至書籍區) (附加在 Clue_bank - Clue_Bank_Content - keyword_area1_1_Collection)
{
    ...
    private string BookName = "《馬相伯先生文集》\n〈馬老先生與輔仁大學〉";

    [SerializeField] private Sprite Book_7_Sprite_1; // 配對書籍封面

    [SerializeField] private Sprite Book_7_Sprite_2; // 配對書籍封面
    ...
    public static int limitNum = 2; // 指定數量為6個(6個填答區域)
    ...
    void Update()
    {
        ...
        // 開啟「重選線索」功能
        if (gameManager.ClueNum_Book7 > 0 && !gameManager.FinishExam_Book7) // 有關鍵字在線索庫中才會有「重選線索」功能
        {
        ...
        if (gameManager.ClueNum_Book7 >= limitNum) // 如果關鍵字蒐集數量達到指定要求(目前設定需要蒐集"限制數量"關鍵字)
        {
            BookTitle.text = BookName + "\n(完成)"; // 更動書籍標題

            BookTitle.color = Color.green; // 字體顏色變為綠色

            BookImage1.sprite = Book_7_Sprite_1; // 於提示庫中顯示配對書籍封面

            BookImage2.sprite = Book_7_Sprite_2;

            gameManager.ClueComplete_Book7 = true; // 記錄該書的線索已經蒐集完畢
        } 
        else
        {
            BookTitle.text = BookName + "\n(" + gameManager.ClueNum_Book7 + "/" + limitNum + ")"; // 顯示關鍵字蒐集進度
        ...
        if (gameManager.Book7_Question1 && gameManager.Book7_Question2) // 如果在CheckPoint視窗中，書籍測驗中的問題全部回答正確後
        {
    ...
    public void Keyword1_return() // 關鍵字歸位 功能 (for每一個關鍵字)
    {
        ...
        if (gameManager.ClueNum_Book7 > 0) // 如果未達指定關鍵字蒐集數量
        {
            gameManager.Book7_Keyword1 = false; // 該關鍵字的蒐集狀態 : false
    ...
    void SnapToCorrectPos_1() // 回答正確
    {
        ...
        gameManager.Book7_Question1 = true; // 書籍1的第一個問題已回答正確
    ...
    void SnapToCorrectPos_2() // 回答正確
    {
        ...
        gameManager.Book7_Question2 = true; // 書籍1的第四個問題已回答正確
